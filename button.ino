...
bool isRed = false; // Status awal: tidak merah (normal)
...
void my_button_event_handler(lv_event_t * e) {
    ...
    lv_obj_t * btn = lv_event_get_target(e); // Ambil objek tombol yang diklik

    if(code == LV_EVENT_CLICKED) {
        isRed = !isRed; // Balikkan status (jika false jadi true, dst)

        if(isRed) {
            // JIKA AKTIF -> UBAH JADI MERAH
            lv_obj_set_style_bg_color(btn, lv_palette_main(LV_PALETTE_RED), LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_label_set_text(objects.tombol_label, "MERAH");
        } else {
            // JIKA NORMAL -> BALIKKAN KE WARNA ASLI (MISAL BIRU)
            lv_obj_set_style_bg_color(btn, lv_palette_main(LV_PALETTE_BLUE), LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_label_set_text(objects.tombol_label, "NORMAL");
        }
    }
}
...
void setup() {
  ...
  if (objects.tombol) {
      lv_obj_add_event_cb(objects.tombol, my_button_event_handler, LV_EVENT_CLICKED, NULL);
  }
